...
fn main() {
    ...
    opts.optopt("c", "config-path", "config file path, uses `default.toml` if not specified", "CONFIG_PATH");
    opts.optflag("b", "benchmark", "Benchmark by rendering the scene multiple times");
    ...
    let back_material = template.build_material(|material| {
        material.diffuse_color = Color::white() * 0.8;
    });
    let back = Plane::new(
    ...
    for (index, mesh) in suzanne.iter_mut().enumerate() {
        let transform = Transform::new(
            Matrix4::scale_uniform(10.0) * Matrix4::rot_y(PI / 2.0) *
